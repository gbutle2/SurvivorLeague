...
DECLARE
  ...
  v_league UUID := 'cccccccc-cccc-cccc-cccc-ccccccccccc1';
  v_other_league UUID := 'cccccccc-cccc-cccc-cccc-ccccccccccc2';
  v_season UUID := 'dddddddd-dddd-dddd-dddd-ddddddddddd1';
...
BEGIN
  ...
  VALUES
    (v_league, 'Test League', 'test-league', 'America/Chicago', v_commish),
    (v_other_league, 'Other League', 'other-league', 'America/Chicago', v_other);
  ...
  INSERT INTO public.league_members (league_id, user_id, role, active) VALUES
    ...
    (v_other_league, v_other, 'commissioner', true);

  INSERT INTO public.seasons (id, league_id, year, status)
  VALUES
    (v_season, v_league, 2026, 'active'),
    ('dddddddd-dddd-dddd-dddd-ddddddddddd2', v_other_league, 2026, 'active');

  INSERT INTO public.weeks (id, season_id, week_number, label, locks_at, status) VALUES
    (v_week_open, v_season, 1, 'Week 1', now() + interval '2 days', 'open'),
    (v_week_locked, v_season, 2, 'Week 2 locked', now() - interval '1 hour', 'locked'),
    (v_week_exact, v_season, 3, 'Week 3 exact', now(), 'open'),
    (v_week2, v_season, 4, 'Week 4', now() + interval '3 days', 'open');

  INSERT INTO public.playoff_rounds (id, season_id, round_number, name, points, locks_at, status) VALUES
    (v_round1, v_season, 1, 'Wild Card', 2, now() + interval '2 days', 'open'),
    (v_round2, v_season, 2, 'Divisional', 4, now() + interval '5 days', 'open');
...
-- 6) Player cannot modify protected fields (result)
SELECT tests.authenticate_as((SELECT player1 FROM test_ids));
SELECT throws_ok(
  format(
...
-- 8) Regular/playoff lists separate: KC used in regular, allowed in playoffs
SELECT tests.authenticate_as((SELECT player1 FROM test_ids));
SELECT lives_ok(
  format(
...
SELECT is(
  (
    WITH u AS (
      ...
      WHERE league_id = (SELECT league FROM test_ids)
        AND user_id = (SELECT player1 FROM test_ids)
      RETURNING 1
    )
    SELECT count(*)::integer FROM u
